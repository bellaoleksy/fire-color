...
# glimpse(bailey_sp)
# library(broom)
# spdf_fortified <- tidy(bailey_sp, region = "DIVISION")
# glimpse(spdf_fortified)
# unique(spdf_fortified$id)
# spdf_fortified_short<- spdf_fortified %>%
#   filter(id %in% c("Prairie Division",
#                    "Hot Continental Division"))
# glimpse(spdf_fortified_short)
# # Plot it
# library(ggplot2)
# ggplot() +
#   geom_polygon(data = spdf_fortified_short, aes( x = long, y = lat, group = group, fill=),
#                fill="blue",color="white") +
#   theme_void() 


##Take the original bailey_sp but narrow it down to the divisions in spdf_fortified_short
...
# bailey_sp <- bailey_sp[grep("DRY", bailey_sp$DOMAIN),]
bailey_sp <- bailey_sp[grep(paste(toMatch,collapse="|"), 
                        bailey_sp$DIVISION),]
# bailey_sp<-rbind(bailey_spA,bailey_spB)
class(bailey_sp)
glimpse(bailey_sp)
...
# 
# pal <- colorNumeric("plasma",  domain=unique(MTBS_mapdf1$year))
# pal2  <- colorFactor("plasma",  domain=unique(bailey_sp$DIVISION))
# leaflet(MTBS_mapdf1) %>%
#   addProviderTiles("CartoDB.Positron") %>%
#   addPolygons(data=bailey_sp,weight=1,col = 'black',
...
#             position = "bottomleft", 
#             title = "Bailey Ecoregion Division",
#             group = "Ecoregions") %>%
#   # addLegend("bottomleft",
#   #           title="Year of burn",
#   #           pal = pal, values = ~year,
#   #           labFormat = labelFormat(big.mark=""),
#   #           group = "Fire Locations") %>%
#     addLayersControl(overlayGroups = c("Fire Locations","Ecoregions"),
#                    options = layersControlOptions(collapsed = FALSE)) %>%
#   setView(-110.8951,38, zoom=4)
# glimpse(bailey_sp)
