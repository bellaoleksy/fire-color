...
##Format and export for JASM talk
gg_title = str_wrap('Number of lakes in burned watersheds', 18)



MTBS_summary %>%
  # pivot_longer(-1)%>%
  ggplot(aes(x=as.numeric(as.character(year)),y=n_lakes_Cat
             # fill=name, color=name, group=name
             ))+
  geom_line(color="#90a955")+
  geom_point(fill="#90a955",color="black",shape=21, size=2.5)+
  ...
  #                   labels=c("Local catchment","Full watershed"),
  #                   name="Legend")+
  geom_smooth(method="gam",se=TRUE, fill="#90a955",color="#90a955")+
...
ggsave('figures/number_lakes_burned_over_time.png', width = 4, height = 5.7, units = "in", bg='transparent')
...
ggsave("figures/n_lakes_burned_by_class.jpg", width = 8, height = 8, units = 'in')
```
...
MTBS_export <- MTBS_long %>%
  select(nhdplusv2_comid, lake_totalarea_ha, lake_lat_decdeg, lake_lon_decdeg) %>%
  distinct(nhdplusv2_comid, .keep_all = TRUE) %>%
  filter(lake_totalarea_ha >= 1)
write_csv(MTBS_export, "data_export/westernUS_lakes.csv")
