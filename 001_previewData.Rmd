...
```{r, echo=FALSE, message=FALSE, warning=FALSE,fig.width = '100%', fig.height=5}
...
HydroLakes_DP_comids <- read.csv("~/Dropbox/dropbox Research/fire-color/data/hydroLakes/HydroLakes_DP_comids.csv") %>%
  rename(nhdplusv2_comid=comid) %>%
  mutate(nhdplusv2_comid=factor(nhdplusv2_comid))

#Read in HydroLakes color stuff
satColor <- read_csv("~/Dropbox/dropbox Research/fire-color/data/hydroLakes/srCorrected_us_hydrolakes_dp_20200628.csv") 
  # rename(nhdplusv2_comid=comid) %>%
  # mutate(nhdplusv2_comid=factor(nhdplusv2_comid))

#Join to MTBS_long
test<-full_join(MTBS_long,HydroLakes_DP_comids,by="nhdplusv2_comid")
