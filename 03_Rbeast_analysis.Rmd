...
Here I am going to run the model and include any lake that burned >= 5% of the local catchment.
```{r, echo=TRUE,  results=FALSE, message=FALSE, warning=FALSE, class.source = 'fold-show'}
...
fire_trend_changes_over5 %>%
  ...
  labs(y="Trend changepoint occurance probability",
       x="COMID",
       title="All lakes where >5 % of the watershed burned")+
...
fire_trend_changes_over5 %>%
  ...
  labs(y="Trend changepoint occurance probability",
       x="COMID",
       title="Where >5 % of the watershed burned AND there are multiple changepoints within 5 years of burn")+
  theme(axis.text.x=element_text(size=6))
...
A look at the lakes that burned >5% of the watershed area and experienced a change in DWL.
...
```{r}
  ...
  fire_trend_changes_over5 %>%
    filter(nhdplusv2_comid %in% c("22560272")) %>%
    left_join(
      .,
      nested_beast %>%
        select(nhdplusv2_comid, data) %>%
  ...
  geom_line(
    ...
    geom_point(aes(color=color))+
    facet_wrap(~nhdplusv2_comid, scale="free_x")+
  labs(title="Walnut Canyon Reservoir, Montana near N. Dakota Border")
```
...
## How many lakes exist in one but not the other?
joined_lakes <- full_join(fireseasonallakes_over5,firetrendlakes_over5, by=c("nhdplusv2_comid","yearBurn"))
...
common_lakes_timeseries %>%
  ggplot()+
  facet_wrap(metric~nhdplusv2_comid, scales="free_y", ncol=11)+
  #Add confidence intervals around changepoint
