...
MTBS_severe<-MTBS_long %>%
  ...
  arrange(yearBurn) %>%
  mutate(n_burns=length(unique(yearBurn)))%>%  #it turns out there are several hundred lakes that burned up to 17 times, likely prescribed burns if I had to guess.
  filter(n_burns==1) %>% #just pull out lakes that burned ONCE
  select(-n_burns)
...
# There are a handful of lakes that burned multiple times; select just the BIGGEST burn which likely has the biggest impact.
  # filter(row_number()==1) #There are a handful of lakes that burned multiple times; select just the FIRST burn which likely has the biggest impact.
...
## Export for Carolina for burn severity analysis
source("scripts/01.0_pullLakeIDsLAGOS.R")
...
burned_lakes <-
  ...
  inner_join(lakeIDs) %>%
  select(nhdplusv2_comid, yearBurn, lake_nhdid, lake_reachcode, lake_namegnis, lake_lat_decdeg,
         lake_lon_decdeg, lake_centroidstate, nhdhr_fdate,
         nhdhr_gnisid, nhdplusv2_reachcode, lagoslakeid)
write.csv(burned_lakes, file="data_export/burned_lakes_carol.csv", row.names = FALSE)
...
#How many lakes that burned over 1% 
...
save(nested_beast, colorModtoSevereBurn_trim, MTBS_moderate, file="data_export/nested_beast_20220719.RData") #This version is 467 lakes that burned over 1% watershed area (WS not CAT) and only lakes that burned ONCE during that time
```
...
base::load('data_export/nested_beast_20220719.RData') #This version is 467 lakes that burned over 1% watershed area (WS not CAT) and only lakes that burned ONCE during that time
```
...
fire_trend_changes_over5 %>%
  ...
  labs(y = "Magnitude of dWL change (nm)",
       x = "Burn severity class (% of watershed)")
...
# 10 years before/after burn (method 1)
...
color_normalized  %>%
  ...
  labs(y=expression(Delta ~ "color (relative to 10-year pre-fire period)"),
       x="Years since fire",
       title="Method 1 - lakes with tcp") 

ggsave("figures/dwl_change_by_sign_allData_method1.png", width = 12, height = 6, units = 'in')
...
ggsave("figures/dwl_change_by_sign_summerMedians_method1.png", width = 12, height = 6, units = 'in')
...
color_normalized_alt  %>%
  mutate(sign=factor(sign,
                     ...
                     levels=c("greener","bluer","no change")))%>%
  ...
  facet_wrap(~sign) + 
  scale_fill_manual(values=c('#33a02c','#1f78b4',"grey50"))+
  scale_x_continuous(breaks=seq(-10,10,1))+
  labs(y=expression(Delta ~ "color (relative to 10-year pre-fire period)"),
       x="Years since fire",
       title="Method 1 - all lakes") +
  theme(legend.position="none")

ggsave("figures/dwl_change_by_sign_allObs_allLakes_method1.png", width = 12, height = 6, units = 'in')
...
yearZero <-
  ...
  ungroup()%>%
  filter(year_norm > -10 & year_norm < 0) %>%
  filter(sign %in% c("greener","bluer","no change")) %>%
  group_by(sign) %>%
  summarize(dwl_norm=median(dwl_norm, na.rm=TRUE)) %>%
    mutate(sign=factor(sign,
                     # labels=c("bluer","greener"),
...
color_normalized_alt  %>%
  ...
  labs(y=expression(Delta ~ "summer color (relative to 10-year pre-fire period)"),
       x="Years since fire",
       title="Method 1 - all lakes") 
ggsave("figures/dwl_change_by_sign_summerMedians_allLakes_method1.png", width = 12, height = 6, units = 'in')
...
# 10 years before/after burn (method 2)
...
color_normalized<-nested_beast %>%
  ...
  mutate(year_norm = case_when(year == yearBurn ~ 0,
                               ...
                               year == yearBurn + 10 ~ 10)) %>%
  ungroup() %>%
  mutate(timeperiod=case_when(year_norm > -10 & year_norm < 0 ~ "pre",
                              TRUE~"other")) %>%
  group_by(nhdplusv2_comid) %>%
  mutate(mean_prefire_dwl=mean(dWL_pred[timeperiod=="pre"], na.rm=TRUE),
         sd_prefire_dwl=sd(dWL_pred[timeperiod=="pre"], na.rm=TRUE),
         dwl_pred_norm=(dWL_pred-mean_prefire_dwl)/sd_prefire_dwl) 
...
yearZero <-
  ...
  filter(year_norm > -10 & year_norm < 0) %>%
  group_by(sign) %>%
  summarize(dwl_pred_norm=mean(dwl_pred_norm))%>%
  mutate(sign=factor(sign,
                     levels=c("positive","negative"),
                     labels=c("greener","bluer"))) 
  # pull()
color_normalized  %>%
  ...
  labs(y=expression(Delta ~ "color (relative to 10-year pre-fire period)"),
       x="Years since fire",
       title="Method 2 - manually calculated z-score - lakes with tcp") 

ggsave("figures/dwl_change_by_sign_allObs_Method2.png", width = 12, height = 6, units = 'in')
...
color_normalized  %>%
  ...
  ggplot(aes(x=year_norm,y=dwl_pred_norm, fill=sign, group=year_norm)) +
  geom_hline(aes(yintercept=dwl_pred_norm, linetype = sign), yearZero, color="black") +
  geom_boxplot(size=0.5, width=0.5, color="black", outlier.shape = NA) +
  facet_wrap(sign~.,
             ncol=4) + 
  scale_fill_manual(values=c('#33a02c','#1f78b4'))+
  scale_x_continuous(breaks=seq(-10,10,1))+
...
ggsave("figures/dwl_change_by_sign_summerMedians_method2.png", width = 12, height = 6, units = 'in')
...
color_normalized_alt<-
  ...
  mutate(pre_post_fire = case_when(year_norm >= -10 & year_norm < 0 ~ "pre",
                                   year_norm > 0 & year_norm <= 5 ~"post")) %>%
  group_by(nhdplusv2_comid) %>%
  mutate(mean_prefire_dwl=mean(dWL[pre_post_fire=="pre"], na.rm=TRUE),
         sd_prefire_dwl=sd(dWL[pre_post_fire=="pre"], na.rm=TRUE),
         dwl_norm=(dWL-mean_prefire_dwl)/sd_prefire_dwl) 
...
sign <-
  ...
  filter(pre_post_fire %in% c("pre","post")) %>% #year zero
  group_by(nhdplusv2_comid, pre_post_fire) %>%
  summarize(dwl_norm=median(dWL, na.rm=TRUE))%>%
  pivot_wider(names_from=pre_post_fire, values_from=dwl_norm) %>%
  mutate(sign=case_when(post > pre ~ "greener",
                        pre > post ~ "bluer",
                        TRUE ~ "no change")) 
...
#Extract the mean normalized color prior to year zero
yearZero <-
  color_normalized_alt %>%
  ungroup() %>%
  filter(sign %in% c("greener","bluer","no change")) %>%
  filter(year_norm > -10 & year_norm < 0) %>%
  group_by(sign) %>%
  mutate(dwl_norm=(dWL-mean_prefire_dwl)/sd_prefire_dwl) %>%
  summarize(dwl_norm=mean(dwl_norm, na.rm=TRUE))
...
color_normalized_alt  %>%
  ...
  labs(y=expression(Delta ~ "color (relative to 10-year pre-fire period)"),
       x="Years since fire",
       title="Method 2 - all lakes") +
  theme(legend.position="none")

ggsave("figures/dwl_change_by_sign_allObs_allLakes_method2.png", width = 12, height = 6, units = 'in')
...
color_normalized_alt  %>%
  ...
  labs(y=expression(Delta ~ "summer color (relative to 10-year pre-fire period)"),
       x="Years since fire",
       title="Method 2 - all lakes") 
ggsave("figures/dwl_change_by_sign_summerMedians_allLakes_method2.png", width = 12, height = 6, units = 'in')
...
manual_sign_df %>%
  ...
  labs(x="Burn severity class (% of local catchment)",
       y="Proportion of lakes",
       title="Of the lakes that burned over 1% WSA - all data",
       subtitle="Median lake color 5-years pre and post fire. Do lakes get greener or bluer?")+
  geom_text(aes(label= paste0("n=", n_lakes_changed)), color="black", vjust=1.3, position = "fill", stat = "identity", size=3)+
  # geom_text(aes(label= paste0(round(prop_change*100,1),"%")), color="black", vjust=2.5, position = "fill", stat = "identity", size=3)+
  geom_hline(yintercept=0.5, color="black", linetype="dashed", size=0.5) 
ggsave("figures/5year_pre_post_manual.jpg", width = 9, height = 7, units = 'in')
...
manual_sign_wide%>%
  ...
  mutate(mode=factor(mode,
                     levels=c("no change","bluer","greener")))%>%
  ggplot(aes(fill = mode, y = prop_change, x = time_period)) +
  geom_point(shape=21)+
  geom_line()+
  scale_fill_manual(values=c("grey70",'#1f78b4','#33a02c'))+
  labs(x="Burn severity class (% of local catchment)",
