...
fire_trend_changes_strict %>%
  ...
  labs(y="Trend changepoint occurance probability",
       x="COMID",
       title="Lakes that burned > 25% of the watershed")+
    scale_fill_manual(values=c("grey20","grey40","grey70","grey90"))+
...
nested_beast %>%
  ...
  labs(y="Observed dominant wavelength (nm)",
       x="Year",
       title="Lakes that burned >25% of the watershed area")+
  cowplot::panel_border() +
  cowplot::background_grid()
...
mcp_subset<-fire_trend_changes_strict %>%
  filter(fire_cp=="fire change") %>%
  group_by(nhdplusv2_comid) %>%
  mutate(changepoint_number = row_number()) %>%
...
##### Closer look at a few more sites
...
For this lake the model detects two very high probability changepoints (>0.9) within 5 years of the burn. It looks like a sharp dip in dWL followed by a quick recovery. LAGOS-US classifies this as a natural lake but I wonder what would cause the lake to go suddenly, but temporarily, blue when it is generally a green lake? 

```{r}


  fire_trend_changes_strict %>%
    filter(nhdplusv2_comid %in% c("14996973")) %>%
    left_join(
      .,
      nested_beast %>%
        select(nhdplusv2_comid, data) %>%
  ...
  geom_line(
    ...
    geom_point(aes(color=color))+
    facet_wrap(~nhdplusv2_comid, scale="free_x")+
  labs(title="Stumpy Meadows Reservoir, Sierra Nevada California (east of Tahoe)")
```

Another case where there is a sudden change followed by a recovery. Not the first time this happened in the record of this lake, so it could very well have something to do with reservoir management, I suppose. 
...
```{r}
  ...
  geom_line(
    ...
    geom_point(aes(color=color))+
    facet_wrap(~nhdplusv2_comid, scale="free_x")+
  labs(title="Dog Lake (reservoir) just north of the CA border in OR")
```

In Dog Lake (reservoir) it's hard to tell but the confidence intervals on the two changepoints near the fire are almost overlapping, that's why it looks like one continuous block. Here's another lake that appears to shift into a greener state following the fire, with no indication of recovering.
