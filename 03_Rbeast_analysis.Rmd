...
length(unique(MTBS_subset$nhdplusv2_comid))
#5410
...
MTBS_subset_export <- MTBS_subset %>%
  dplyr::select(nhdplusv2_comid, lake_nhdid, nhdplusv2_reachcode, lake_lat_decdeg, lake_lon_decdeg, lake_waterarea_ha, lake_perimeter_m)
...
#Example for change in seasonality/variability... 
dat_23061964 <- dat %>%
  filter(nhdplusv2_comid == "23061964") %>%
  unnest(data) %>%
  left_join(., MTBS_severe %>%
              select(nhdplusv2_comid, yearBurn), by = "nhdplusv2_comid") 

#Example of Clear Lake, CA but we don'thave 5 years of post-fire data.
dat_15032791 <- dat %>%
  filter(nhdplusv2_comid == "15032791") %>%
  unnest(data) %>%
...
# 1. Open jpeg file
png("figures/JASM/Rbeast_demo.png", width = 900, height = 1600, res=300)
plot(
     o, 
     # index = 1,
     vars  = c('st','t','tcp','o','ocp'),  
     col         = NULL, 
     main        = "Big Lake, Arizona",
     xlab        = 'Time',
     ylab        = NULL,
     cex.main    = 1,
     cex.lab     = 1,
...
o=beast.irreg(dat_23061964$value, time=dat_23061964$date,deltat=1/12, freq=12)
plot(
     o, 
     # index = 1,
     vars  =  c('st','t','tcp','torder','o','ocp','error'),
     col         = NULL, 
     main        = "Black Lake, WA",
     xlab        = 'Time',
     ylab        = NULL,
...
o=beast.irreg(dat_15032791$value, time=dat_15032791$date,deltat=1/12, freq=12)
plot(
     o, 
     # index = 1,
     vars  =  c('st','t','tcp','torder','o','ocp','error'),
...
colorModtoSevereBurn_trim <- colorModtoSevereBurn %>%
  ...
  filter(value>=5) %>% #Lakes over 5% burn
  mutate(dWL = replace(dWL, dWL < 470, NA),
         dWL = replace(dWL, dWL > 583, NA)) %>%
  drop_na(dWL) %>%
  group_by(nhdplusv2_comid, year) %>%
...
gc()#Free up memory
end_time <- Sys.time()
save(nested_beast, file="data_export/nested_beast_20220505.RData")
```

```{r}
# base::load('fire-color/data_export/nested_beast_20220226.RData')
# base::load('data_export/nested_beast_20220226.RData')
# base::load('data_export/nested_beast_20220412.RData')
base::load('data_export/nested_beast_20220505.RData') #only difference here from earlier version is only includes burns over 5% of watershed area
```
...
ggsave("figures/04b_all_lakes_with_one_changpt.jpg", width = 18, height = 16, units = 'in')
...
pred_dwl_change %>%
  ...
  geom_smooth(method="lm",se=FALSE)+
  facet_wrap(~sign, scales="free_y")
#Looks a little better than raw values but still no relationship with % burn.
...
# Look at a lake with a high cpPR but low cpAbruptChange
  ...
  geom_line(
    ...
    geom_point(aes(color=color))+
    facet_wrap(~nhdplusv2_comid, scale="free_x")+
  labs(title="Willow Lake (Reservoir), OR")
...
# color_normalized_alt <-nested_beast %>%
#         select(nhdplusv2_comid, pred_tcps) %>%
#         unnest(c(pred_tcps)) %>%
#         filter(nhdplusv2_comid %in% fire_trend_changes_over5$nhdplusv2_comid) %>%
#   left_join(., fire_trend_changes_over5 %>%
...
#                                year == yearBurn + 10 ~ 10)) %>%
#   group_by(nhdplusv2_comid) %>%
#   mutate(fire_period = case_when(year_norm < 0 ~ "before",
#                                  year_norm > 0 ~ "after")) %>%
#   group_by(nhdplusv2_comid, fire_period) %>%
#   mutate(mean_dWL=mean(dWL_pred,na.rm=TRUE),
#          sd_dWL=sd(dWL_pred, na.rm=TRUE)) %>%
#   pivot_wider(names_from = fire_period, values_from = c(mean_dWL,sd_dWL)) %>%
#   arrange(mean_dWL_before) %>%
#   mutate(mean_dWL_before = mean_dWL_before[1]) %>%#fill in NA gaps with the 'before' value for each lake
#   arrange(sd_dWL_before) %>%
#   mutate(sd_dWL_before = sd_dWL_before[1]) %>% #fill in NA gaps with the 'before' sd value for each lake
#   select(-contains("_NA"),-contains("after")) %>%
#   group_by(nhdplusv2_comid) %>%
#   mutate(dwl_pred_zscore = (dWL_pred-mean_dWL_before)/sd_dWL_before,
#          dwl_pred_norm = scale(dwl_pred_zscore),
#          dwl_pred_original = scale(dWL_pred))
# 
# data$zScore<-(data$value-mean(data$value[data$group==1]))/sd(data$value[data$group==1])
# 
# 
# #How do new z-score and old scaled values compare?
# color_normalized_alt %>%
#   mutate(fire_period = case_when(year_norm < 0 ~ "before",
#                                  year_norm > 0 ~ "after")) %>%
#   mutate(dwl_pred_norm_z=scale(dwl_pred_norm)) %>%
#   ggplot(aes(x=dwl_pred_norm_z,y=dwl_pred_original,fill=fire_period))+
...
color_normalized  %>%
  ...
  geom_hline(aes(yintercept=dwl_pred_norm, linetype = sign), yearZero, color="white") +
  geom_boxplot(size=0.5, width=0.5, color="white") +
  facet_wrap(~sign) + 
...
ggsave("figures/JASM/dwl_change_by_sign_allData.png", width = 12, height = 6, units = 'in')
...
#### Visualized burned lakes (w/o sudden chngpt)
```{r}
...
color_normalized_alt  %>%
  ...
  geom_hline(aes(yintercept=dwl_norm, linetype = sign), yearZero, color="white") +
...
ggsave("figures/JASM/dwl_change_by_sign_allObs_allLakes.png", width = 12, height = 6, units = 'in')
...
color_normalized_alt  %>%
  ...
  scale_fill_manual(values=c('#1f78b4','#33a02c'))+
    scale_x_continuous(breaks=seq(-10,10,1))+
  labs(y=expression(Delta ~ "dWL (relative to 10-year pre-fire period)"),
       x="Years since fire",
...
##ANNUAL MEDIAN - new way of standardizing the data
#Extract the median normalized color at year zero
yearZero <-
  color_normalized_alt %>%
  group_by(nhdplusv2_comid, year_norm, sign) %>%
...
color_normalized_alt  %>%
  ...
  scale_x_continuous(breaks=seq(-10,10,1))+
  labs(
    # y="Annual median dwl (normalized by lake)",
       x="Years since fire",
       y=expression(Delta ~ "dWL (relative to 10-year pre-fire period)"),
       title="Annual median")+
  theme(axis.text.x = element_text(
        angle = 0,
        hjust = 1,
